...
const tvId = ref(props.id || (route.params.id as string))
const tvShow = ref<any>(null)
const isLoading = ref(true)

const loadTvDetails = async () => {
  try {
    const result = await tvStore.discoverTv('1')
    if (result.result) {
      const tvShows = (result as any).data.results
      tvShow.value = tvShows.find((tv: any) => tv.id.toString() === tvId.value) || null
    }
  } catch (error) {
...
watch(
  ...
  (newTvShow) => {
    if (newTvShow) {
      backdropUrl.value = newTvShow.backdrop_path ? image('w780', newTvShow.backdrop_path) : ''
      posterUrl.value = newTvShow.poster_path
        ? image('w342', newTvShow.poster_path)
...
<template>
  <div class="tv-details-view">
      ...
      <div class="container">
        <div class="tv-content">
          <div class="poster-section">
            <img :src="posterUrl" :alt="tvShow.name" class="poster" />
          </div>
          ...
          <div class="info-section">
            ...
            <div class="meta-info">
              <div class="rating">
                {{ tvShow.vote_average?.toFixed(1) }} ({{ tvShow.vote_count }} votos)
              </div>
              <div class="first-air-date" v-if="tvShow.first_air_date">
                Estreou em {{ new Date(tvShow.first_air_date).toLocaleDateString('pt-BR') }}
              </div>
            </div>

            <div class="genres" v-if="tvShow.genre_ids">
              <span v-for="genreId in tvShow.genre_ids" :key="genreId" class="genre-tag">
                {{ tvStore.genreName(genreId) }}
              </span>
            </div>

            <div class="overview">
              <h3>Sinopse</h3>
              <p>{{ tvShow.overview || 'Sinopse não disponível.' }}</p>
            </div>

            <div class="additional-info">
              <div class="info-item">
                <strong>Idioma Original:</strong> {{ tvShow.original_language?.toUpperCase() }}
              </div>
              <div class="info-item">
                <strong>Popularidade:</strong> {{ tvShow.popularity?.toFixed(0) }}
              </div>
              <div class="info-item" v-if="tvShow.origin_country?.length">
                <strong>País de Origem:</strong> {{ tvShow.origin_country.join(', ') }}
              </div>
            </div>
          </div>
        </div>
    ...
    <div v-else class="not-found">
      ...
      <p>A série que você está procurando não existe ou foi removida.</p>
    </div>
  </div>
</template>
